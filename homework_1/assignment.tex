...
def get_risk_ratio(n1: int, n2: int) -> None:
    ...
    print(f"risk_ratio = {pi1_hat / pi2_hat}")
    print(f"log_risk_ratio = {log(pi1_hat / pi2_hat)}")
\end{verbatim}
...
\item[(b)] Simulate 1,000,000 sample proportions with $n_1=n_2=1000$.

\begin{verbatim}
y1 = binomial(1000, 0.20, 1_000_000)
y2 = binomial(1000, 0.10, 1_000_000)
\end{verbatim}
...
\item[(c)] Simulate 1,000,000 sample proportions from each treatment with $n_1=30$ and $n_2=70$. What do the empirical sampling distributions suggest about the normality? Is this as clear as what you found for part (a)?

\begin{verbatim}
y1 = binomial(30, 0.20, 1_000_000)
y2 = binomial(70, 0.10, 1_000_000)
\end{verbatim}

The empirical sampling distribution of $\hat{\pi}_1/\hat{\pi}_2$ remains skewed.  
The log relative risk again appears closer to normality.

However, because the sample sizes are smaller and unequal, the deviation from normality is more noticeable than in part (a).

\item[(d)] Simulate 1,000,000 sample proportions from each treatment with $n_1=70$ and $n_2=30$. What do the empirical sampling distributions suggest about the normality?.

\begin{verbatim}
y1 = binomial(70, 0.20, 1_000_000)
y2 = binomial(30, 0.10, 1_000_000)
\end{verbatim}

The same general pattern is observed.  
The relative risk remains skewed, while the log relative risk appears more symmetric and closer to a normal distribution.

Thus, across all scenarios considered, the logarithm of the relative risk provides a sampling distribution that is closer to normal.
